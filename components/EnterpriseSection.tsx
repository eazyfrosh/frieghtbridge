...
export function EnterpriseSection() {
  return (
    <section id="enterprise" className="section relative overflow-hidden bg-brand-500 text-white">
      <div aria-hidden="true" className="pointer-events-none absolute inset-0">
        <div className="absolute inset-0 bg-[radial-gradient(90%_70%_at_10%_0%,#C24500_0%,transparent_55%),radial-gradient(70%_60%_at_95%_100%,#C24500_0%,transparent_55%)] opacity-60" />
...
function StatCard({ value, suffix, label }: { value: number; suffix: string; label: string }) {
  ...
  return (
    <motion.div
      ...
      className="group relative overflow-hidden rounded-3xl border border-white/25 bg-white/[0.12] p-5 backdrop-blur transition-colors duration-400 ease-premium hover:border-white/45 hover:bg-white/20 sm:p-7"
    >
      <span
        aria-hidden="true"
        className="pointer-events-none absolute -right-10 -top-10 h-28 w-28 rounded-full bg-white/25 blur-2xl transition-opacity duration-500 group-hover:opacity-100 md:opacity-0"
      />
