...
export function PageHero({ eyebrow, title, description, breadcrumb, children, overlap }: PageHeroProps) {
  return (
    <section
      className={`relative isolate overflow-hidden bg-brand-500 pt-[124px] text-ink-950 lg:pt-[148px] ${
        overlap ? 'pb-32 sm:pb-40' : 'pb-16 sm:pb-20 lg:pb-24'
      }`}
    >
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute inset-0 bg-[radial-gradient(100%_80%_at_20%_-20%,#FF8A22_0%,transparent_58%),radial-gradient(70%_60%_at_95%_0%,#FFA758_0%,transparent_55%)] opacity-65" />
      ...
      <div className="on-dark container">
        <Reveal>
          <nav aria-label="Breadcrumb" className="mb-7">
            <ol className="flex items-center gap-1.5 text-sm text-ink-950/80">
              <li>
        ...
        <Reveal delay={0.1}>
          <h1 className="display-1 mt-6 max-w-3xl text-ink-950">{title}</h1>
        </Reveal>

        {description && (
          <Reveal delay={0.16}>
            <p className="mt-6 max-w-2xl text-base leading-relaxed text-ink-800 sm:text-lg">{description}</p>
          </Reveal>
        )}
