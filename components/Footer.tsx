  ...
  return (
    <footer className="on-dark relative overflow-hidden bg-ink-950 text-ink-300">
      <div aria-hidden="true" className="pointer-events-none absolute inset-0">
        <div className="absolute inset-0 bg-grid-dark bg-[size:56px_56px] opacity-25" />
        <div className="absolute -top-32 left-1/4 h-72 w-72 rounded-full bg-brand-600/15 blur-[110px]" />
      </div>
      ...
      <div className="container relative">
        <div className="grid gap-12 py-16 lg:grid-cols-[1.35fr_2.65fr] lg:gap-16 lg:py-20">
          ...
          <div>
            <Logo tone="light" />
            <p className="mt-5 max-w-sm text-[0.95rem] leading-relaxed text-ink-400">
            ...
            <ul className="mt-7 space-y-3 text-sm">
              ...
              <li className="flex items-center gap-2.5">
                ...
                <a href={`mailto:${SITE.email}`} className="text-ink-300 transition-colors hover:text-white">
                  {SITE.email}
                </a>
              </li>
            </ul>
                ...
                <ul className="mt-5 space-y-3">
                  {column.links.map((link) => (
                    <li key={link.label}>
                      <Link
                        href={link.href}
                        className="text-[0.92rem] text-ink-300 transition-colors duration-200 hover:text-white"
                      >
                        {link.label}
                      </Link>
                    </li>
        ...
        <div className="flex flex-col items-start justify-between gap-6 border-t border-white/10 py-7 sm:flex-row sm:items-center">
          <p className="text-sm text-ink-400">© 2026 FreightBridge. All rights reserved.</p>

          <div className="flex items-center gap-4">
            <p className="hidden text-sm text-ink-400 sm:block">{SITE.tagline}</p>
              ...
              {SOCIALS.map((social) => (
                <li key={social.label}>
                  <a
                    ...
                    className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/12 bg-white/5 text-ink-300 transition-all duration-300 ease-premium hover:-translate-y-0.5 hover:border-white/30 hover:bg-white/10 hover:text-white"
                  >
